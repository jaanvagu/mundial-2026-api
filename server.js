...
function normalizeWorldCupMatch(item, generatedAt) {
  ...
  return {
    ...
    home_name: canonicalTeamName(firstString(item.home_name, item.homeTeam, item.home_team, item.team_home_name, item.home_team_name_en), firstString(item.home_code, item.home_team_code, item.home_short)),
...
function normalizeWhenIsKickoffMatch(item, generatedAt) {
  ...
  return {
    ...
    home_name: canonicalTeamName(firstString(item.home_name), firstString(item.home_code)),
...
function normalizeEspnEvent(event, generatedAt) {
  ...
  return {
    ...
    home_name: canonicalTeamName(firstString(home.team && home.team.displayName, home.team && home.team.name), firstString(home.team && home.team.abbreviation, home.team && home.team.shortDisplayName)),
    away_name: canonicalTeamName(firstString(away.team && away.team.displayName, away.team && away.team.name), firstString(away.team && away.team.abbreviation, away.team && away.team.shortDisplayName)),
...
function mergeMatch(primary, fallback, generatedAt) {
  ...
  const resolved = {
    ...
    home_name: canonicalTeamName(coalesce(primary && primary.home_name, fallback && fallback.home_name), coalesce(primary && primary.home_code, fallback && fallback.home_code)),
    away_name: canonicalTeamName(coalesce(primary && primary.away_name, fallback && fallback.away_name), coalesce(primary && primary.away_code, fallback && fallback.away_code)),
...
function enrichMatchesWithEspn(matches, espnMatches, generatedAt) {
  ...
  return matches.map((match) => {
    ...
    return {
      ...
      home_name: canonicalTeamName(match.home_name || espnMatch.home_name || null, match.home_code || espnMatch.home_code || null),
      away_name: canonicalTeamName(match.away_name || espnMatch.away_name || null, match.away_code || espnMatch.away_code || null),
